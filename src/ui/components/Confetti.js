...
class Confetti extends React.Component {
    animate(particles) {
        this.animationId = window.requestAnimationFrame(this.draw.bind(this, particles));
    }
    draw(particles) {
        const ctx = this.ctx;
        const width = WIDTH;
        ...
        if (ctx) {
            ...
            particles.forEach((particle, index) => {
                const { deltaX, deltaY, color, radius, opacity, deltaOpacity } = particle;
                ...
                ctx.lineTo(particle.x + particle.tilt, particle.y + particle.tilt + particle.radius);
                ctx.stroke();
                ctx.closePath();
    ...
    render() {
        return (React.createElement("canvas", { className: `confetti-canvas ${this.props.className}`, width: WIDTH, height: HEIGHT, ref: canvas => {
                if (canvas) {
                    this.canvas = canvas;
